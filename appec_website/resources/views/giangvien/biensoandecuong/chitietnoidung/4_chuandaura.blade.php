...
<table class="table table-bordered">
    <thead>
        <th colspan="2"></th>
        <th style="background-color: green">{{ __("CDIO's SOs") }}</th>
        <th style="background-color: green">{{ __("ABET's SOs") }}</th>
                ...
                <td colspan="3">
                    ...
                    <div class="modal fade" id="chuDe_{{ $cdr1->maCDR1VB }}" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <form action="{{ asset('/giang-vien/bien-soan-de-cuong/them_chuan_dau_ra_mon_hoc') }}"
                                method="post">
                                @csrf
            ...
            @foreach ($kqht as $x)
                @if ($x->maCDR1 == $cdr1->maCDR1)
                        ...
                        <tr>
                            ...
                            <td rowspan={{ $rs }}>
                                {{-- <a title="Delete" class="btn btn-danger"
                                    ...
                                    href="{{ asset('/giang-vien/bien-soan-de-cuong/xoa-ket-qua-hoc-tap-mon-hoc/'.$x->maKQHT) }}">
                                    <i class="fa fa-trash"></i> LO
                                </a>
                                <!-- Button trigger modal -->
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add_cdio_{{ $x->maKQHT }}">
                                    <i class="fas fa-plus"></i> CDIO
                                </button> --}}
                                
                                <!-- Modal -->
                                <div class="modal fade" id="add_cdio_{{ $x->maKQHT }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <form action="{{ asset('/giang-vien/bien-soan-de-cuong/them-ket-qua-hoc-tap-chuan-dau-ra') }}" method="post">
                                        @csrf
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Add') }} CDIO</h5>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                ...
                                                <div class="modal-body">
                                                    <label for=""> {{ __('Level-3 outcome') }}:</label>
                                                    <select name="maCDR3" id="" class="form-control select-item">
                                                        @foreach ($cdr as $t)
                                                            @if ($t->maCDR1 == $cdr1->maCDR1)
                                                                <option value="{{ $t->maCDR3 }}" > {{ $t->maCDR3VB }} -
                                                                    {{ $t->tenCDR3 }}</option>
                            ...
                            {{-- <td>
                                <div class="btn-group">
                                    ...
                                    <a title="Delete" class="btn btn-danger"
                                        onclick="return confirm('Confirm?')"
                                        href="{{ asset('/giang-vien/bien-soan-de-cuong/xoa_chuan_dau_ra_mon_hoc/'.$x->id) }}">
                    ...
                    <div class="modal fade" id="editKQHT_HP_{{ $x->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <form action="{{ asset('/giang-vien/bien-soan-de-cuong/sua_chuan_dau_ra_mon_hoc') }}"
                            method="post">
                            @csrf
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
