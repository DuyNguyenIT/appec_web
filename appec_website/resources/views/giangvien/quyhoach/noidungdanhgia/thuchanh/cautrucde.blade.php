          ...
          <div class="row">
            ...
            <div class="col-12">
                  ...
                  @foreach ($noidung as $data)
                      ...
                      <b>Phương án trả lời:</b> <br>
                        @for ($k = 0; $k < count($data->phuongAn); $k++)
                          [{{ $k+1 }}] ({{ $data->phuongAn[$k]->diemPA }} điểm)(ABET:{{ $data->phuongAn[$k]->maChuanAbetVB }})(CDR3:{{ $data->phuongAn[$k]->maCDR3VB }}): 
                          <button type="button" class="btn btn-success" data-toggle="modal" data-target="#editPA_{{ $data->phuongAn[$k]->id }}">
                            <li class="fas fa-edit"></li>
                          </button>
                              {!! $data->phuongAn[$k]->noiDungPA !!} 

                          <!-- Modal -->
                            <div class="modal fade" id="editPA_{{ $data->phuongAn[$k]->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                              <div class="modal-dialog modal-lg" role="document">
                                <form action="{{ asset('/giang-vien/quy-hoach-danh-gia/noi-dung-danh-gia/xem-noi-dung-danh-gia/sua-phuong-an-thuc-hanh') }}" method="post">
                                @csrf
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Chỉnh sửa phương án</h5>
                                  ...
                                  <div class="modal-body">
                                    <div class="form-group">
                                      <input type="text" name="id" value="{{ $data->phuongAn[$k]->id }}" hidden>
                                    </div>
                                    <div class="form-group">
                                      <textarea name="noiDungPA" id="edit_PA_{{$data->phuongAn[$k]->id}}" cols="30" rows="10">
                                      {!! $data->phuongAn[$k]->noiDungPA !!}
                                      </textarea>
                                      <script>
                                        CKEDITOR.replace( "edit_PA_"+ {{$data->phuongAn[$k]->id}}, {
                                    ...
                                    <div class="from-group">
                                      ...
                                      <select name="maChuanAbet" id="" class="form-control">
                                        @foreach ($abet as $ab)
                                          ...
                                          <option value="{{ $ab->maChuanAbet }}" selected>{{ $ab->maChuanAbetVB }}--{{ $ab->tenChuanAbet }}</option>
                                          @else
                                          <option value="{{ $ab->maChuanAbet}}">{{ $ab->maChuanAbetVB }}--{{ $ab->tenChuanAbet }}</option>
                                          @endif
                                        @endforeach
                                      </select>
                                    </div>
                                    <div class="form-group">
                                      ...
                                      <input type="text" name="diemPA" value="{{ $data->phuongAn[$k]->diemPA }}" class="form-control">
                                    </div>
                                    <div class="from-group">
                                      ...
                                      <select name="maCDR3" id="" class="form-control">
                                        @foreach ($cdr3 as $cd)
                                          ...
                                          <option value="{{ $cd->maCDR3 }}" selected>{{ $cd->maCDR3VB }}--{{ $cd->tenCDR3 }}</option>
                                          @else
                                          <option value="{{ $cd->maCDR3}}">{{ $cd->maCDR3VB }}--{{ $cd->tenCDR3 }}</option>
                                          @endif
                                        @endforeach
                                      </select>
                                    </div>
              ...
              <div class="card card-info card-outline">
                <div class="card-body">
                  <h5 class="card-title"><h3>Adding new content form</h3></h5>
