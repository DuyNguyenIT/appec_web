...
<table id="" class="tatable table-bordered table-hover table-responsive table-striped">
    <thead>
        <tr>
            ...
            <th class="second" rowspan="2">{{ __('Year') }}</th>
            <th class="second" rowspan="2">{{ __('Semester') }}</th>
            <th class="second" rowspan="2">{{ __('Class') }}</th>
            <th class="second" rowspan="2">{{ __('Course') }}</th>
            {{-- <th class="fourth" colspan="{{ count($arr_thongkeKQ[0]) }}">SO</th> --}}
            @foreach ($CDR3 as $cdr3)
                <th class="fourth" colspan="5">{{ $cdr3->maCDR3VB }}</th>
            @endforeach
        </tr>
         <tr>
            @foreach ($CDR3 as $cdr3)
                <th class="fourth">A</th>
                <th class="fourth">B</th>
                <th class="fourth">C</th>
                <th class="fourth">D</th>
                <th class="fourth">Fail</th>
            @endforeach
        </tr> 
    </thead>
    <tbody>
