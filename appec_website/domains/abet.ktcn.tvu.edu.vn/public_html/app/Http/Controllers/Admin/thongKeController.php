...
use App\Exports\thongKeCTExport;
use App\Models\hocPhan_ctDaoTao;
use App\Exports\thongKeABETExport;
...
class thongKeController extends Controller
{
    ...
    public function export_ct(Excel $excel,$maCT)
    {
        Session::put('maCT',$maCT);
        $ctdt=ctDaoTao::find($maCT);
        $filename=CommonController::con_str($ctdt->tenCT);
        return $excel->download(new thongKeCTExport,'thongKeCT_'.$filename.'.xlsx');
    }

    public function export_abet(Excel $excel,$maCT)
    {
        Session::put('maCT',$maCT);
        $ctdt=ctDaoTao::find($maCT);
        $filename=CommonController::con_str($ctdt->tenCT);
        return $excel->download(new thongKeABETExport,'thongKeABET_'.$filename.'.xlsx');
    }
