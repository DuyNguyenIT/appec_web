...
class GVPhanBienDeCuongController extends Controller
{
    public function index(Type $var = null)
    {
        $phanBien=phan_bien_de_cuong::where('maGV',session::get('maGV'))->pluck('maHocPhan');
        $hocphan=hocPhan::whereIn('maHocPhan',$phanBien)->orderBy('maHocPhan','desc')->with('hocphan_ctdt')->get();
        ...
        return view('giangvien.phanbiendecuong.dshocphan',compact('hocphan','ctkhoi','ctdt','loaihp'));
    }
    ...
    {   
        if(phan_bien_de_cuong::where('maGV',session::get('maGV'))->where('maHocPhan',$maHocPhan)->count('maHocPhan')==0){
            return redirect('/giang-vien/phan-bien-de-cuong');
        }
        $phanBien=phan_bien_de_cuong::where('maGV',session::get('maGV'))->where('maHocPhan',$maHocPhan)->first();
        $now = Carbon::now();
        $start=Carbon::parse($phanBien->thoiGianBatDau)->format('Y-m-d h:m:s');
        $end=Carbon::parse($phanBien->thoiGianKetThuc)->format('Y-m-d h:m:s');
        ...
        return view('giangvien.phanbiendecuong.noidungdecuong',['hocPhan'=>$hocPhan,'monHoc'=>$mon,'hocphan_ctdaotao'=>$hocphan_ctdaotao,
    ...
    public function them_mon_tien_quyet(Request $request)
    {
        ...
        return redirect('/giang-vien/phan-bien-de-cuong/chinh-de-cuong/'.session::get('maHocPhan'));
    }
