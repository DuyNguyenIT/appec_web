<h5><b>1. {{ __('General information') }}</b></h5>
<!-- ----------------------------------1. Thông tin chung-------------------- -->
    ...
    <tr>
        ...
        <td><i>
                @foreach ($monTQ as $data) 
                    {{ $data->hoc_phan->tenHocPhan }} <a title="Delete" class="btn btn-danger"
                    onclick="return confirm('Confirm?')"
                    href="{{ asset('/quan-ly/hoc-phan/de-cuong-mon-hoc/xoa-mon-tien-quyet/'.$data->id) }}">
        ...
        <td>
            ...
            <div class="modal fade" id="addMonTQ" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <form action="{{ asset('/quan-ly/hoc-phan/de-cuong-mon-hoc/them_mon_tien_quyet') }}"
                        method="post">
                        @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="maMonTienQuyet" id="" class="form-control">
                                        @foreach ($monHoc as $data)
                                            <option value="{{ $data->maHocPhan }}"> {{ $data->maHocPhan }} --
                                                {{ $data->tenHocPhan }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
        ...
        <td>
            ...
            <div class="modal fade" id="addyeuCau" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <form action="{{ asset('/quan-ly/hoc-phan/de-cuong-mon-hoc/sua_yeu_cau_mon_hoc') }}"
                        method="post">
                        @csrf
                        <div class="modal-content">
                            <div class="modal-header">
