                                            ...
                                            @foreach ($users as $u)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <span
                                                            class="badge badge-success">{{ $text_array[$u->permission - 1] }}</span>
                                                    </td>
                                                    <td>
                                                        @if ($u->isBlock == 1)
                                                    ...
                                                    <td>
                                                        <button title="Edit" class="btn btn-primary" data-toggle="modal"
                                                            data-target="#edit_{{ $u->username }}">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        @if ($u->isBlock == 0)
                                                            <a title="Block"
                                                                href="{{ asset('quan-ly/tai-khoan/khoa/' . $u->username) }}"
                                                                class="btn btn-warning"><i class="fa fa-lock"></i></a>
                                                        @else
                                                            <a title="unBlock"
                                                                href="{{ asset('quan-ly/tai-khoan/mo-khoa/' . $u->username) }}"
                                                                class="btn btn-info"><i class="fas fa-lock-open"></i></a>
                                                        @endif
                                                        <a title="Delete" class="btn btn-danger"
                                                            onclick="return confirm('Do you want to delete {{ $u->username }}?')"
                                                            href="{{ asset('quan-ly/tai-khoan/xoa/' . $u->username) }}"><i
                                                                class="fa fa-trash"></i></a>

                                                        <div class="modal fade" id="edit_{{ $u->username }}"
                                                            tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                                                            aria-hidden="true">
                                                            <div class="modal-dialog" role="document">
                                                                <form action="{{ asset('quan-ly/tai-khoan/sua') }}"
                                                                    method="post">
                                                                    @csrf
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            ...
                                                                            <div class="form-group">
                                                                                ...
                                                                                <select name="permission" id=""
                                                                                    ...
                                                                                    @for ($i = 0; $i < count($text_array); $i++)
                                                                                        @if ($u->permission == $i + 1) <option
                                                                                        value="{{ $i + 1 }}"
                                                                                        selected>{{ $text_array[$i] }}</option>
                                                                                    @else
                                                                                        <option
                                                                                        value="{{ $i + 1 }}">{{ $text_array[$i] }}</option> @endif
                                                                                    @endfor
                                                                                </select>
                                                                            </div>
                                                                            <div class="form-group">
